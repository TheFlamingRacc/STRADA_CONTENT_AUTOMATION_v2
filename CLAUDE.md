...
│   └── YouTubeService.js     — Пошук відео, транскрипт, YouTube embed
├── analytics/
...
- Алгоритм: пошук відео по рандомній темі з `AUTO_TOPICS` → YouTube Data API v3 → отримання транскрипту через `youtube-transcript` (npm, безплатний, без API ключа) → генерація поста Gemini → публікація
- Якщо транскрипт недоступний — використовується опис відео
- **Gemini** генерує пост в режимі персонажа (persona) або редактора: заінтриговує, але НЕ переказує все відео
...
- Embed вставляється **між** першим і другим абзацом тексту (текст до і після відео)
- Команда для ручного запуску: `npm run youtube`
- `YouTubeService.AUTO_TOPICS` — 15 автомобільних тем для ротації
- `YOUTUBE_API_KEY` потрібен тільки для пошуку (`findRandomAutoVideo`); транскрипт — без ключа
...
- О **00:01** (Київ) генерується розклад на новий день — пости, engagement-слоти і YouTube-слоти
...
- Рандомна кількість YouTube-постів: `YOUTUBE_POSTS_PER_DAY_MIN`–`YOUTUBE_POSTS_PER_DAY_MAX` (1–3)
- Активні години для всіх: `ACTIVE_HOUR_START`–`ACTIVE_HOUR_END` (8:00–23:00 Київ)
- Cron перевіряє поточну хвилину щохвилини → якщо є слот (пост, engagement, YouTube) → запускає відповідну задачу
...
- `data/youtube_channels.json` — список YouTube каналів для пошуку. Редагується вручну, комітиться в репо. Для додавання каналу: знайти Channel ID на YouTube (Про канал → три крапки → Поділитись → Копіювати ідентифікатор) і додати рядок у JSON.
...
| 6 | Немає моніторингу квоти YouTube API | Додати лічильник і throttling |
