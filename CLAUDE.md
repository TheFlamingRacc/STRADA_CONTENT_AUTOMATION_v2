...
- Слоти генеруються при старті бота і щоночі о 00:01 разом з розкладом постів
- Кожна сесія — **один рандомний юзер, одна взаємодія** (лайк або збереження)
- Алгоритм: логін → завантаження **всіх сторінок** `/feed/all` (по 21 пост) → вибір дії (лайк з шансом `1 - SAVE_CHANCE`, збереження з шансом `SAVE_CHANCE`) → фільтр: тільки пости де `is_liked: false` або `saved_post: false` відповідно (перевірка per-user, а не загальна) → рандомний вибір → дія → логаут
- Якщо кандидатів за основною дією немає — автоматично пробує протилежну
- Затримка між юзерами: `ENGAGEMENT_DELAY_MIN_MS`×2 – `ENGAGEMENT_DELAY_MAX_MS`×2 (6–16 сек)
- Discord логування: повідомлення на початок сесії + окреме повідомлення на **кожну взаємодію** з іменем юзера і посиланням на пост
...
- **1 взаємодія на сесію:** кожен слот = один рандомний юзер робить одну дію (лайк або збереження)
