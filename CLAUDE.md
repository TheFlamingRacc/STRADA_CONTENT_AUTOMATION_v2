...
- Відфільтровує зображення: дрібні (<20KB), з лого/аватарами, низькою роздільною здатністю
...
- **Gemini** генерує пост від імені персонажа з `user.prompt` (характер, мова, стиль)
...
- Фото завантажується на **Strada CDN** і вставляється в HTML-контент поста
...
- **Фільтрація контенту:** Gemini відсіює не-авто статті; PostService — дрібні фото (<20KB)
...
| 4 | `UserProfiler` аналізує інтереси але не використовує їх при відборі статей | Підключити фільтрацію черги за профілем юзера |
...
*Паспорт згенеровано на основі вихідного коду STRADA_CONTENT_AUTOMATION_v2. Актуально на: квітень 2025.*
